"""Tushare P0 - 每日涨跌停价格 stk_limit，按交易日拉取"""
...
SAVE_DIR = r'E:\ashare_similarity_runtime\data\cache\prediction\tushare\stk_limit'
...
def pull():
    ...
    print(f"stk_limit: done {len(done)}, remaining {len(remaining)}")
    ...
    for i, td in enumerate(remaining):
        try:
            ...
            time.sleep(2 ** errors)
    save_progress(progress)
    print(f"stk_limit done! Total: {progress['total_rows']}")
