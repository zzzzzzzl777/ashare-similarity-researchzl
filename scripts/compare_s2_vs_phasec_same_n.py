"""Same-N comparison: both models rank by RAW probability, take identical top-N, compare accuracy."""
...
april = april[(april["limit_up_like"] != 1) & (april["short_phase_days_3"] >= 1)]

s2_raw = run_inference(s2, april)
pc_raw = run_inference(pc, april)
actual = april["actual"].values
dates = april["date"].values

# Method 1: Global top-N (take top N from entire April by raw prob)
print("=" * 75)
...
for n in [100, 200, 300, 467, 500, 630, 700, 812, 1000, 1500, 2000, 3000]:
    if n > len(actual):
        continue
...
# Method 2: Daily top-K (same K per day, raw prob)
print(f"\n{'=' * 75}")
...
# Method 3: overlap analysis at top-500
print(f"\n{'=' * 75}")
