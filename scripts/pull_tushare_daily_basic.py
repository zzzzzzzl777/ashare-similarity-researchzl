"""Tushare P2 - daily_basic 每日基础指标，按交易日拉取"""
...
SAVE_DIR = r'E:\ashare_similarity_runtime\data\cache\prediction\tushare\daily_basic'
...
def pull():
    ...
    print(f"daily_basic: done {len(done)}, remaining {len(remaining)}")
    ...
    for i, td in enumerate(remaining):
        try:
            ...
            time.sleep(2 ** errors)
    save_progress(progress)
    print(f"daily_basic done! Total: {progress['total_rows']}")
