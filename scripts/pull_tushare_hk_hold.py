"""Tushare P1 - 沪深港通持股 hk_hold，按交易日拉取"""
...
SAVE_DIR = r'E:\ashare_similarity_runtime\data\cache\prediction\tushare\hk_hold'
...
def pull():
    ...
    print(f"hk_hold: done {len(done)}, remaining {len(remaining)}")
    ...
    for i, td in enumerate(remaining):
        try:
            ...
            time.sleep(2 ** errors)
    save_progress(progress)
    print(f"hk_hold done! Total: {progress['total_rows']}")
