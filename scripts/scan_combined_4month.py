"""Find strategies with highest COMBINED return across all 4 months (sel + holdout)."""
import sys
...
# Combine into one frame for 4-month total evaluation
combined = pd.concat([pc_sel, pc_hold], ignore_index=True)
print(f"Combined: {len(combined)} rows, {combined['date'].nunique()} days, months: {sorted(combined['month'].unique())}")
...
print(f"\n{'='*90}")
print(f"  TOP 30 — COMBINED 4-month Total Return (2023-02~04 + 2026-04)")
print(f"{'='*90}")
...
print(f"\n\n{'='*90}")
print(f"  MONTHLY DETAIL — Top 5")
print(f"{'='*90}")
seen = set()
count = 0
for r in results:
    key = (r['score_col'], r['threshold'], r['top_n'], r['recipe'], r['exit'], r['filter'])
