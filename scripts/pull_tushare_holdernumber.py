"""Tushare P0 - 股东户数 stk_holdernumber，按交易日拉取
数据按季报披露，非每日更新，同一季度内数值不变
"""
...
SAVE_DIR = r'E:\ashare_similarity_runtime\data\cache\prediction\tushare\stk_holdernumber'
...
def pull():
    ...
    print(f"stk_holdernumber: done {len(done)}, remaining {len(remaining)}")

    errors = 0
    for i, td in enumerate(remaining):
        try:
            df = pro.stk_holdernumber(trade_date=td)
            ...
            time.sleep(2 ** errors)
    save_progress(progress)
    print(f"stk_holdernumber done! Total: {progress['total_rows']}")
