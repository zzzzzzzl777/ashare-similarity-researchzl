"""Tushare P1 - 融资融券明细 margin_detail，按交易日拉取"""
...
SAVE_DIR = r'E:\ashare_similarity_runtime\data\cache\prediction\tushare\margin_detail'
...
def pull():
    ...
    print(f"margin_detail: done {len(done)}, remaining {len(remaining)}")
    ...
    for i, td in enumerate(remaining):
        try:
            ...
            time.sleep(2 ** errors)
    save_progress(progress)
    print(f"margin_detail done! Total: {progress['total_rows']}")
