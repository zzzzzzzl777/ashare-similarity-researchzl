"""Tushare P0 - 涨跌停明细 limit_list_d，按交易日拉取"""
...
SAVE_DIR = r'E:\ashare_similarity_runtime\data\cache\prediction\tushare\limit_list_d'
...
def pull():
    ...
    print(f"limit_list_d: done {len(done)}, remaining {len(remaining)}")
    ...
    for i, td in enumerate(remaining):
        try:
            ...
            time.sleep(2 ** errors)
    save_progress(progress)
    print(f"limit_list_d done! Total: {progress['total_rows']}")
