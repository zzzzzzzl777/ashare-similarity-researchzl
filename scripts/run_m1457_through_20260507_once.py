...
TABLE_PATH = REPORT_DIR / "m1457_greedy_top8_through_20260507_T078_top6_ties_table.csv"
    ...
    table = pd.DataFrame(
        {
            ...
            "next_high_return_pct": output["next_high_return_pct"].map(fmt_num),
            "next_close_return_pct": output["next_close_return_pct"].map(fmt_num),
            "close": output["close"].map(fmt_num),
            "\u6362\u624b": output["turnover"].map(fmt_num),
        }
    )
    table.to_csv(TABLE_PATH, index=False, encoding="utf-8-sig")
    print(f"TABLE {TABLE_PATH} rows={len(table)}", flush=True)
    print("TABLE_TAIL_BEGIN", flush=True)
    print(table.tail(40).to_csv(index=False), flush=True)
    print("TABLE_TAIL_END", flush=True)
    return 0
